...
void  word_print(char *str){
    ...
    for(int i = 0; i < len; i++){
        if (word_start) {
            if (str[i] == SPACE_CHAR){
                // Just reach the space after the word ended.
                word_start = false;
                printf(" (%d)\n", wlen);
                wlen = 0;
            } else {
...
int main(int argc, char *argv[]){
    ...
    switch (opt){
        ...
        case 'r':
            ...
            // The string is reversed in place to stay consistent with storage space.
            // No new array was needed since the original was simply modified.
            break;
        case 'w':
            printf("Word Print\n----------\n");
